...
class SwipableCellLayouter {
    ...
    private var cellTranslationX: CGFloat {
        get {
            item.contentView.frame.origin.x
        }
        set {
    ...
    init(
        item: SwipableActionsItem, layout: CollectionSwipableCellLayout?,
        ...
        maxActionsVisibleWidth =
            layout?.swipingAreaWidth() ?? kDefaultActionsWidth
        setupViews()

        // prevent reset contentView frame before delete animation
        contentViewObservation = item.contentView.observe(\.frame) {
            [weak self] (view, change) in
            guard let `self` = self else {
                return
            }
            MainActor.assumeIsolated {
                if self.prevContentViewOrigin.x < 0
    ...
    private func onSwipe(prevValue: CGFloat) {
        ...
        var expectedFinishType:
            (finishType: FinishAnimationType, collectOffset: Bool)
        ...
        let oppositeBounceRange =
            direction == .leftToRight
            ? 0...CGFloat.infinity : -CGFloat.infinity...0
        let openBounceRange =
            direction == .leftToRight
            ? -CGFloat.infinity ... -maxActionsVisibleWidth
            : maxActionsVisibleWidth...CGFloat.infinity

        let swipeFullOpenRange =
            -CGFloat.infinity ... -item.view.bounds.width * 0.8

        let sector: Sector

        if oppositeBounceRange.contains(cellTranslationX) || swipePosition == 0
        {  // for case of closing from full open
            sector = .bounceClose
        } else if openBounceRange.contains(cellTranslationX)
            && !swipeFullOpenRange.contains(swipePosition)
        {
            sector = .bounceOpen
        } else if openBounceRange.contains(cellTranslationX)
            && swipeFullOpenRange.contains(swipePosition)
            ...
            offsetValue = (
                directionFactor
                    * easeOut(
                        ...
                        endValue: -item.view.bounds.width,
                        asymptote: -maxActionsVisibleWidth + -item.view.bounds
                            .width / 6), previousSector == .fullOpen
            )
        ...
        if expectedFinishType.collectOffset {
            offsetCollector.add(
                offset: abs(swipePosition - prevValue),
                for: expectedFinishType.finishType)

            let limitOffset = item.view.bounds.width * kCompletionOffsetFactor
            switch (
                expectedFinishType.finishType,
                offsetCollector.offset(for: expectedFinishType.finishType)
            ) {
            case (.fullOpen, _),
    ...
    private func setupViews() {
        ...
        if wrapperView == nil {
            ...
            actionsViewWrapper.autoresizingMask = [
                .flexibleWidth, .flexibleHeight,
            ]
            item.view.insertSubview(
                actionsViewWrapper, belowSubview: item.contentView)
    ...
    private func layoutActions() {
        ...
        if direction == .leftToRight {
            ...
            containerView.frame = CGRect(
                x: item.view.bounds.width - width, y: 0, width: width,
                height: item.view.bounds.height)
        } else {
            containerView.autoresizingMask = [
                .flexibleRightMargin, .flexibleHeight,
            ]
            containerView.frame = CGRect(
                x: 0, y: 0, width: width, height: item.view.bounds.height)
        }

        layout?.layoutActionsView()
    }

    private func performFinishAnimation(
        toValue value: CGFloat, completion: (@MainActor () -> Void)? = nil
    ) {
        ...
        UIView.animateKeyframes(
            ...
            animations: {
                ...
                let frame1 = FrameInfo(
                    duration: 0.4,
                    value: value >= 0
                        ? value + bounceValue * 0.9 : value - bounceValue * 0.9
                )
                let frame2 = FrameInfo(
                    duration: 0.2,
                    value: value >= 0
                        ? value + bounceValue : value - bounceValue
                )
                ...
                for frameInfo in frameInfos {
                    UIView.addKeyframe(
                        withRelativeStartTime: frameStart,
                        relativeDuration: frameInfo.duration,
                        animations: {
    ...
    private func swipeWidth() -> CGFloat {
        item.view.bounds.width + (layout?.swipingAreaInset() ?? 0)
    }
...
private func easeOut(
    value: CGFloat, startValue: CGFloat, endValue: CGFloat, asymptote: CGFloat
) -> CGFloat {
